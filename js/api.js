...
api.getInventoryIndex = async function () {
  const { data: products, error } = await supabaseClient.from('products').select('*, product_variants(*), sub_category:sub_category_id(code, name), main_category:main_category_id(code, name), supplier:supplier_id(id, name)').is('deleted_at', null);
  if (error) throw error;
  ...
  products.forEach(function (p) {
    result.products[p.code] = {
      ...
      supplierId: p.supplier ? p.supplier.id : null,
...
api.recordStandaloneSupplierReturn = async function (session, payload) {
  const { data, error } = await supabaseClient.rpc('rpc_record_standalone_supplier_return', {
    p_supplier_name: payload.supplierName,
    p_items: payload.items.map(function (i) { return { variant_code: i.variantCode, qty: i.qty }; }),
    p_settlement: payload.settlement === 'خصم' ? 'خصم' : 'نقدي',
    p_treasury_account_id: payload.treasuryAccountId || null
  });
  if (error) throw error;
  return data;

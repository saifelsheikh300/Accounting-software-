-- ============================================================
-- الدفعة 49: رصيد افتتاحي للمخزون — بمنتجات حقيقية ومربوطة
-- بمورد، بس من غير ما تتحول لعملية شراء عادية بتخصم من الخزنة
-- أو تحط مديونية غلط على مورد مش مديونة له.
--
-- الفرق عن أوردر الشراء العادي: المبلغ مش هيتقسم "مدفوع/آجل"
-- (مفيش دفع حقيقي بيحصل دلوقتي أصلاً)، هيتقسم "مديون بيه فعلاً
-- للمورد" مقابل "متغطي من رأس مالك الحالي (مش مديونة بيه)".
-- الجزء المديون بيه بس هو اللي يدخل حساب "الموردون"، والباقي
-- بيدخل "رصيد افتتاحي" — والخزنة مبتتأثرش خالص في الحالتين.
...
begin
  ...
  values (v_order_number, v_supplier_id, v_total,
    case when v_owed = 0 then 'مدفوع بالكامل' when v_owed = v_total then 'آجل بالكامل' else 'مدفوع جزئيًا' end,
